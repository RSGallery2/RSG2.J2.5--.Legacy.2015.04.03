...
class rsgTagsManager{
	...
	**/
	...
	function get( $id = null ){
		...
		$gallery = rsgTagsManager::_get( $id );
    ...
    function getArray( $cid ){
        ...
        foreach( $cid as $gid ){
            $galleries[] = rsgTagsManager::_get( $gid );
        }
        return $galleries;
    }
    ...
    function getList( $parent ){
        ...
        $database->setQuery("SELECT * FROM #__rsgallery2_tags".
                            " WHERE parent = '$parent'".
    ...
    function deleteArray( $cid ){
        ...
        foreach( $cid as $gid ){
            ...
            $rsgAccess->checkGallery( 'delete', $gid ) or die("RSGallery2: Access denied to delete tag $tag");
        }

        // delete all galleries and sub galleries
        $tags = rsgTagsManager::_getArray( $cid );

        return rsgTagsManager::_deleteTree( $tags );
    }
	...
	*/
	function _get( $tag ){
		...
		if( !isset( $galleries[$gallery] )){
			...
			if( count($row)==0 && $gallery!=0 ){
				...
				return rsgTagsManager::_getRootGallery();
			}
			$row = $row[0];
    ...
    function _deleteTree( $galleries ){
        ...
        foreach( $galleries as $gallery ){
            rsgTagsManager::_deleteTree( $gallery->kids() );
            ...
            $query = "DELETE FROM #__rsgallery2_tags WHERE id = $id";
