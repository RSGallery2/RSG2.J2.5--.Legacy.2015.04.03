...
/**
*****************************************
functions below this point do not appear to be used.  they are depreciated and will be removed shortly
*****************************************
**/
...
function showMainPage() {
	...
	//Initiate rsgTemplate
	$tpl = new rsgTemplate();
	
	//Get gallery ID from URL
	$gid 	= mosGetParam( $_REQUEST, 'catid', 0 );
	$inline = mosGetParam( $_REQUEST, 'page', 0 );
	...
	//Include the right template file
	if (!$gid) {
		include(JPATH_RSGALLERY2_SITE. DS . 'tpl' . DS . $cur_template . DS . 'main.php');
	}
	
	if ($gid && $inline) {
		include(JPATH_RSGALLERY2_SITE. DS . 'tpl' . DS . $cur_template . DS . 'display.php');
	}
	
	if ($gid && !$inline) {
		include(JPATH_RSGALLERY2_SITE. DS . 'tpl' . DS . $cur_template . DS . 'thumbs.php');
	}
	if ($rsgConfig->get('displayBranding')) {
		$tpl->showRsgFooter();
	}
}

/*
function showMainPageX(){
    global $rsgConfig, $mainframe;
	//Initiate rsgTemplate
	$tpl = new rsgTemplate();
