...
class html_rsg2_templates{
	...
	*/
	function showTemplates( &$rows, &$pageNav, $option ) {
		global $my, $mosConfig_live_site, $rsgOption;
		...
		for ( $i=0, $n = count( $rows ); $i < $n; $i++ ) {
			$row = &$rows[$i];
			$link = "index2.php?option=$option&rsgOption=$rsgOption&task=edit&hidemainmenu=1&cid[]=". $row->directory;
			?>
			<tr class="<?php echo 'row'. $k; ?>">
				...
				<td>
				<a href="<?php echo $link;?>" onmouseover="showInfo('<?php echo $row->name;?>','<?php echo $row->directory; ?>')" onmouseout="return nd();">
				<?php echo $row->name;?>
				</a>
				</td>
				<td align="center">
